...
type Props = { points?: number } // points must be a NUMBER

export default function GraphSection({ points = 0 }: Props) {
  const ref = useRef<SVGPathElement>(null)

  // Always render a visible flat path first so it never appears empty
  ...
  useEffect(() => {
    ...
    // Flat mode: 0 points -> draw once and skip RAF
    if (!(points > 0)) {
    ...
    // 100 => 10, 150 => 13, 200 => 16, then +3 each additional +50 points  (= +0.06 per point after 100)
    const pulsePower = points <= 100 ? 10 : 10 + 0.06 * (points - 100)

    const pulseDepth = Math.min(0.15 + points * 0.004, 0.7)  // how hard it hits
    ...
    const animate = () => {
      ...
      const envelope = Math.pow(pulsePhase, pulsePower)      // short, strong bursts

      const amp = baseAmp * (1 + pulseDepth * envelope)      // amplitude with pulse
      const t2 = t * (0.7 + Math.min(points / 200, 0.6))     // slight secondary phase speed
    ...
    // draw immediately once, then start animating
    ref.current?.setAttribute('d', FLAT_D)
    raf = requestAnimationFrame(animate)
    return () => cancelAnimationFrame(raf)
  }, [points, FLAT_D])

  const flat = !(points > 0) // style hint only
      ...
      <div className="mx-auto max-w-6xl px-4 py-14">
        ...
        <div className="rounded-2xl bg-gradient-to-b from-[#191B20] to-[#111218] p-4 shadow-[0_20px_50px_rgba(0,0,0,0.45)] border border-white/10">
          <svg viewBox="0 0 1000 400" className="w-full h-[320px] block">
            <defs>
              {/* absolute gradient coords to avoid “invisible” strokes on flat lines */}
