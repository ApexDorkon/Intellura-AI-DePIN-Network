...
export default function GraphSection({ points = 0, muted = false }: Props) {
  ...
  // Default flat path so we never render an empty path
  ...
  useEffect(() => {
    ...
    // Ensure we draw something immediately
    ref.current?.setAttribute('d', FLAT_D)
    raf = requestAnimationFrame(animate)
    return () => cancelAnimationFrame(raf)
  }, [points, muted, FLAT_D])

  const isFlatNow = muted || points <= 0
      ...
      <div className="mx-auto max-w-6xl px-4 py-14">
        ...
        <div className="rounded-2xl bg-gradient-to-b from-[#191B20] to-[#111218] p-4 shadow-[0_20px_50px_rgba(0,0,0,0.45)] border border-white/10">
          <svg viewBox="0 0 1000 400" className="w-full h-[320px] block">
            <defs>
              {/* Use absolute coords to avoid “invisible stroke” edge cases */}
            ...
            <path
              ref={ref}
              d={FLAT_D}
              stroke={isFlatNow ? '#00E5FF' : 'url(#g1)'}  
              strokeWidth="2.5"
              strokeLinecap="round"
